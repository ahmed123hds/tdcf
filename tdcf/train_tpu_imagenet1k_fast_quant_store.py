"""TPU trainer for the fast sharded int8 DCT store."""
...
from tdcf.fast_quant_store import FastQuantizedDCTStore, FastShardBatchSampler
...
def parse_args():
    p = argparse.ArgumentParser("Fast Quantized DCT Store ImageNet TPU Trainer")
...
def build_model(args, device, img_size):
    if args.backbone == "resnet50":
        ...
        backbone = tv_models.vit_b_16(weights=None, image_size=img_size)
...
def budget_ratio(args, epoch):
    if not args.budget_mode:
        return 1.0
    T = max(args.epochs - 1, 1)
    t = min(epoch, T)
    return args.beta + (args.max_beta - args.beta) * (t / T) ** args.gamma


def make_loader(store, batch_size, world_size, rank, shuffle, drop_last, seed):
    sampler = FastShardBatchSampler(
        num_samples=store.N,
        records_per_shard=store.records_per_shard,
    ...
    loader = DataLoader(store.get_dataset(), batch_sampler=sampler, num_workers=0, pin_memory=False)
...
def train_process(index, args):
    ...
    val_loader, _val_sampler = make_loader(
    ...
    for ep in range(start_epoch, args.epochs):
        ...
        train_store.set_budget_ratio(ratio)
        ...
        va_n_total = xm.mesh_reduce(f"fastq_va_n_{ep}", va_n_accum, sum)
        va_corr_total = xm.mesh_reduce(f"fastq_va_corr_{ep}", va_corr_accum, sum)
        va_loss_total = xm.mesh_reduce(f"fastq_va_loss_{ep}", va_loss_accum, sum)
        ...
        if is_master:
            print(
                ...
                f"PhysIO={train_store.get_io_ratio():.3f} | "
...
def train_process_entry(index, args):
    try:
        ...
        print(f"[rank {index}] Unhandled exception in fast quant worker", flush=True)
        traceback.print_exc()
        raise
