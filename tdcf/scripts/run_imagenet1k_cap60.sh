...
SAVE_DIR="${SAVE_DIR:-./results/imagenet1k_cap60_mild}"
BETA="${BETA:-0.5}"
...
while [ $attempt -lt $MAX_RETRIES ]; do
    ...
    python3 -m tdcf.train_tpu_imagenet1k \
        ...
        --save_dir "$SAVE_DIR" \
        $RESUME_FLAG \
        2>&1 | tee -a "logs/cap60_run_attempt_${attempt}.log" \
    && break

    EXIT_CODE=$?
    echo "[$(date)] Training exited with code $EXIT_CODE."

    if grep -q "Training complete" "logs/cap60_run_attempt_${attempt}.log" 2>/dev/null; then
