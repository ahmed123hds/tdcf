...
SAVE_DIR='./results/imagenet1k_cap100'
MAX_RETRIES=20
RETRY_WAIT=30
...
while [ $attempt -lt $MAX_RETRIES ]; do
    ...
    python3 -m tdcf.train_tpu_imagenet1k \
        ...
        --budget_mode --beta 1.0 --max_beta 1.0 --gamma 1.0 \
        --k_low 1 \
        --save_dir "$SAVE_DIR" \
        $RESUME_FLAG \
        2>&1 | tee -a "logs/cap100_run_attempt_${attempt}.log" \
    && break

    EXIT_CODE=$?
    echo "[$(date)] Training exited with code $EXIT_CODE."

    if grep -q "Training complete" "logs/cap100_run_attempt_${attempt}.log" 2>/dev/null; then
