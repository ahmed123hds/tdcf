#!/bin/bash
# =============================================================================
# Auto-restart wrapper for ImageNet-1K crop-aware store TDCF cap90 training.
# Usage: bash tdcf/scripts/run_imagenet1k_store_cap90.sh
# =============================================================================
...
SAVE_DIR='./results/imagenet1k_store_cap90'
MAX_RETRIES=20
RETRY_WAIT=30
...
while [ $attempt -lt $MAX_RETRIES ]; do
    ...
    python3 -m tdcf.train_tpu_imagenet1k_store \
        ...
        --amp_bf16 \
        --budget_mode --beta 0.4 --max_beta 0.9 --gamma 1.5 \
        --k_low 1 \
        --save_dir "$SAVE_DIR" \
        $RESUME_FLAG \
        2>&1 | tee -a "logs/store_cap90_attempt_${attempt}.log" \
    && break

    EXIT_CODE=$?
    echo "[$(date)] Training exited with code $EXIT_CODE."
    if grep -q "Training complete" "logs/store_cap90_attempt_${attempt}.log" 2>/dev/null; then
