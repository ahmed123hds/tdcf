...
def generate_dashboard(results_path, out_path):
    ...
    ht = data["tdcf"]
    hb = data.get("baseline")
    
    # Mock objects for the plotting function
    class MockEst:
    ...
    ax.plot(ht["epoch"], ht["test_acc"], "o-", c=c_tdcf, lw=2.5, ms=4, label="TDCF (Adaptive)")
    if hb: ax.plot(hb["epoch"], hb["test_acc"], "s--", c=c_base, lw=2, ms=4, label="Baseline (Full)")
    ...
    ax.plot(ht["epoch"], ht["train_loss"], "o-", c=c_tdcf, lw=2, ms=4)
    ...
    ax.plot(ht["epoch"], ht["wall_s"], "o-", c=c_tdcf, lw=2, ms=4)
